...
static const u8 adp5589_regmap_defaults_02[ADP5589_MAX_REG + 1] = {
	...
	/* 0x08 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* 0x10 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* 0x18 */ 0x00, 0x41, 0x01, 0x00, 0x11, 0x24/*0x04*/, 0x00, 0x00,
	/* 0x20 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* 0x28 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* 0x30 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* 0x38 */ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
no_keypad:
	...
	ret = adp5585_keys_reset_parse(adp5585);
	dev_info(dev,
		 "keypad pins:0x%lx, pwm:%d, reset1:%d, reset2:%d, row5:%d\n",
		 *adp5585->keypad, adp5585->has_pwm, adp5585->nkeys_reset1,
		 adp5585->nkeys_reset2, adp5585->info->has_row5);
	return ret;
}
...
static void adp5585_report_events(struct adp5585_dev *adp5585, int ev_cnt)
{
	...
	for (i = 0; i < ev_cnt; i++) {
		...
		key_press = FIELD_GET(ADP5585_KEY_PRESS_MASK, key);
...
static irqreturn_t adp5585_irq(int irq, void *data)
{
	...
	dev_warn_ratelimited(adp5585->dev, "Got IRQ %u\n", status);

	if (status & ADP5585_OVRFLOW_INT)
		dev_err(adp5585->dev, "Event Overflow Error\n");
...
static int adp5585_setup(const struct adp5585_dev *adp5585)
{
	...
	for (i = 0; i < adp5585->nkeys_unlock; i++) {
		ret = regmap_write(adp5585->regmap, ADP5589_UNLOCK1 + i,
				   adp5585->unlock_keys[i] | ADP5585_KEV_EV_PRESSED);
		if (ret)
			return ret;
	}
	...
	for (i = 0; i < adp5585->nkeys_reset1; i++) {
		ret = regmap_write(adp5585->regmap, regs->reset1_event_a + i,
				   adp5585->reset1_keys[i] | ADP5585_KEV_EV_PRESSED);
		if (ret)
			return ret;
	}
	...
	for (i = 0; i < adp5585->nkeys_reset2; i++) {
		ret = regmap_write(adp5585->regmap, regs->reset2_event_a + i,
				   adp5585->reset2_keys[i] | ADP5585_KEV_EV_PRESSED);
		if (ret)
			return ret;
	}
	...
	return regmap_write(adp5585->regmap, regs->poll_ptime_cfg,
			      ...
			      struct adp5585_dev *adp5585)
{
	...
	ret = regmap_write(adp5585->regmap, adp5585->info->regs->gen_cfg,
			   ADP5585_OSC_FREQ_500KHZ | ADP5585_INT_CFG |
			   ADP5585_OSC_EN);
	if (ret)
		return ret;

	ret = regmap_write(adp5585->regmap, adp5585->info->regs->int_en,
